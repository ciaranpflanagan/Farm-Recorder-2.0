...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($_POST['mother_tag_number'])) {
        $errors[] = 'You forgot to enter the mothers tag number!';
    }
    else
    {
        $mtn = mysqli_real_escape_string($dbc, trim($_POST['mother_tag_number']));
    }

    // Checking for a ram number
    if (empty($_POST['ram_number'])) {
        $errors[] = 'You forgot to enter the rams number!';
    }
    else
    {
        $rn = mysqli_real_escape_string($dbc, trim($_POST['ram_number']));
    }
    $id = $_SESSION['user_id'];
...
if (empty($errors)) {
    ...
    // Making the query
    $q = "INSERT INTO test_1234 (user_id, tag_number, mother_tag_number, ram_number) VALUES ('$id', '$tn', '$mtn', '$rn');";
